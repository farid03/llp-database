...
- на родителя
- на первого ребенка (начало двусвязного списка детей)
- на предыдущий элемент двусвязного списка детей родителя
- на следующий элемент двусвязного списка детей родителя
...
### Модули программы
В программе присутствует несколько модулей, на взаимодействии которых основана ее работа:
* [commands](commands) -- содержит реализации функций публичного интерфейса для взаимодействия пользователя 
с модулем хранения данных, структуры для хранения информации о запросе пользователя и 
[структура для возврата результатов](commands/iterator) find-функций.
* [data_process_utils](data_process_utils) -- функции для валидации структур принимаемых со стороны 
пользовательского интерфейса.
* [dbstruct](data_process_utils) -- структуры для хранения данных в файле и функции для их чтения/записи.
* [file_workers](file_workers) -- функции инкапсулирующие базовое взаимодействие с файлом.
...
* Осуществляет **поиск** в дереве документов **по** заданному **условию**.
    ...
    ```
* **Получить все узлы** дерева документов.
    ```c++
...
## Тесты
Конфигурация тестирующей машины - Ubuntu 22.04, ядро 5.15.0-56-generic, AMD Ryzen 5 3500U, 8Gb RAM.

Проект был скомпилирован при помощи g++ 11.3.0, с флагом оптимизации -O3.
